...
def Create_annotation_for_BLIP(image_folder, outpath, np_index=None):
    ...
    try:
        ...
        file_names.sort(key=lambda x: int(x[-10:-5]))#sort
...
def main():
    ...
    # with open(f'/ssd7.7tb2/gzy/Show-o/T2I-CompBench/examples/dataset/color_val.txt', "r") as f:
    #     validation_prompts = f.read().splitlines()

    # all_data = os.listdir(os.path.join(args.out_dir,"samples_ours"))
    # val_data = []
    # for item in all_data:
    #     if item.split('_')[0] in validation_prompts:
    #         val_data.append(item)
    #     else:
    #         # print("rm -rf \"" + os.path.join(args.out_dir, "samples", item) + "\"")
    #         os.system("rm -rf \"" + os.path.join(args.out_dir, "samples_ours", item) + "\"")
